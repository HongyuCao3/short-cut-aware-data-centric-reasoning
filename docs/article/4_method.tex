...
\label{eq:gradient_surgery}
    ...
    \end{cases}
\end{equation}
where $\gamma \in [0,1]$ is the projection strength ($\gamma{=}1$ fully removes the conflicting component). This conflict-gated rule is deliberately asymmetric: even a shortcut-flagged sample may carry a small positive alignment with $\mathbf{g}_{\mathcal{V}}$, and erasing that component would discard a validation-improving direction---the opposite of our goal. Projection fires only in the truly harmful case ($\mathbf{g}_s$ points against validation), and the resulting $\mathbf{g}_s'$ then has non-negative inner product with $\mathbf{g}_{\mathcal{V}}$.
